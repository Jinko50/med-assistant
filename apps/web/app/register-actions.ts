...
export async function registerAccount(_state:{message:string},form:FormData){
 const parsed=z.object({email:z.email().max(254),password:z.string().min(12).max(128)}).safeParse({email:form.get('email'),password:form.get('password')});
 if(!parsed.success)return {message:'Enter a valid email and a password of at least 12 characters.'};
