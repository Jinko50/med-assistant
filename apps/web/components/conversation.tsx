...
//   * A failure never eats the message. The text stays in the box and the chosen file stays
//     attached, and Retry resumes from where it stopped - a file that already reached
//     Storage is not sent a second time.
...
type Phase = '' | 'uploading' | 'uploaded' | 'sending';
...
export function Conversation({ patient, locale, initial, unprepared = false }: {
  ...
  // A file that already reached Storage. Kept across a failed send so Retry does not
  // upload 40 MB again.
  ...
  useEffect(() => { bottom.current?.scrollIntoView({ block: 'end' }); }, [messages.length, phase]);

  // Returns the stored document id, or '' when there is nothing to upload, or throws the
  // translated failure key.
  async function uploadIfNeeded(): Promise<string> {
    ...
    setPhase('uploading'); setPercent(0);
    ...
    await new Promise<void>((resolve, reject) => {
      ...
      xhr.onload = () => xhr.status >= 200 && xhr.status < 300 ? resolve() : reject(new Error(xhr.status === 413 ? 'storageRejected' : 'interrupted'));
    ...
    // The bytes have arrived. They have NOT been read: finishUpload only re-reads the
    // header to confirm the format, and nothing opens the document's contents.
    setPhase('uploaded');
    ...
    setUploadedId(started.documentId);
  ...
  async function send() {
    ...
    if (!text && !file && !uploadedId) { setFailure('sendEmpty'); return; }
    setFailure('');
    let documentId = '';
    try {
      documentId = await uploadIfNeeded();
    } catch (error) {
      // The upload failed. The text and the file are both still here; the person is told
      // what happened and can press the same button again.
      setPhase(''); setPercent(0);
      setFailure((error as Error).message || 'interrupted');
      return;
    }
    setPhase('sending');
    const form = new FormData();
    form.set('locale', locale); form.set('patient', patient); form.set('text', text);
    form.set('documentId', documentId);
    form.set('attachment', documentId ? 'stored' : file ? 'failed' : 'none');
    const result = await sendMessage({ message: '' }, form);
    setPhase(''); setPercent(0);
    if (result.message || !result.added) { setFailure(result.message || 'sendUnavailable'); return; }
    setMessages(previous => [...previous, ...result.added!]);
    setDraft(''); setFile(null); setUploadedId('');
    if (fileInput.current) fileInput.current.value = '';
  }

  // Upload failures come from the document vocabulary, send failures from the chat one.
  const failureText = failure
    ? (failure in (t.chatMessages as Record<string, string>)
      ? actionMessage(locale, 'chatMessages', failure)
      : actionMessage(locale, 'documentMessages', failure))
    : '';

  const progress = phase === 'uploading' ? `${t.chatUploadSending}${percent ? ` ${percent}%` : ''}`
    : phase === 'uploaded' ? t.chatUploadStored
      : phase === 'sending' ? t.chatSending : '';
      ...
      <label className="composer-attach">
        ...
        <input ref={fileInput} type="file" name="file" accept="application/pdf,image/jpeg,image/png"
          onChange={event => {
            ...
            setFile(event.target.files?.[0] ?? null); setUploadedId(''); setFailure('');
          }}/>
      </label>
      <textarea name="text" rows={2} value={draft} placeholder={t.chatPlaceholder} maxLength={4000}
        aria-label={t.chatPlaceholder} onChange={event => setDraft(event.target.value)}
        onKeyDown={event => {
          if (event.key === 'Enter' && !event.shiftKey) { event.preventDefault(); void send(); }
        }}/>
      <button className="button primary composer-send" type="submit" disabled={phase !== ''}>
        {phase ? t.chatSending : t.chatSend}
      </button>
    </form>
    {file && <p className="tiny composer-file" dir="ltr">
      {t.chatAttachment}: {file.name}
      <button className="text-link" type="button" onClick={() => {
        setFile(null); setUploadedId('');
        if (fileInput.current) fileInput.current.value = '';
